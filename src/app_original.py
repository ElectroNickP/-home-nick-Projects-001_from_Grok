...
# Добавляем фильтр для конвертации timestamp в datetime
...
def api_v2_auth_required(f):
    """Декоратор для проверки как HTTP Basic Authentication, так и сессий в API v2"""
    import base64
    
    @wraps(f)
    def decorated_function(*args, **kwargs):
        # Сначала проверяем сессию (для веб-интерфейса)
        if "user_id" in session:
            return f(*args, **kwargs)
            
        # Затем проверяем HTTP Basic Auth (для API клиентов)
        ...
        try:
            ...
            return jsonify({"error": "Unauthorized", "message": "Invalid authentication format"}), 401
...
def update_application():
    ...
    try:
        ...
        if result["success"]:
            ...
            return jsonify({"error": result["error"], "backup_id": result.get("backup_id")}), 500
...
def validate_input_data(data):
    ...
    max_lengths = {
        "bot_name": 100,
        "telegram_token": 100,
        "openai_api_key": 200,  # Увеличено для OpenAI API ключей
        "assistant_id": 100,
    ...
    for field, value in data.items():
        if isinstance(value, str):
            ...
            if field in max_lengths and len(value) > max_lengths[field]:
                return (
                    False,
                    f"Поле {field} слишком длинное (максимум {max_lengths[field]} символов)",
                )
            ...
            if field == "openai_api_key":
                # Поддержка как старых (sk-...), так и новых (sk-proj-...) форматов ключей
                if not re.match(r"^sk-(proj-)?[A-Za-z0-9_-]{20,}$", value):
...
def get_all_bots_v2():
    ...
    try:
        ...
        if search:
            filtered_bots = [
                bot for bot in filtered_bots if search.lower() in bot["config"]["bot_name"].lower()
            ]
...
def get_bot_v2(bot_id):
    ...
    try:
        ...
        def serialize_bot_enhanced(bot_entry):
            ...
            bot_name = bot_entry["config"].get("name") or bot_entry["config"].get("bot_name") or f"Bot {bot_entry['id']}"
...
def restart_bot_v2(bot_id):
    ...
    try:
        ...
        if start_success:
            return api_response(
                {"bot_id": bot_id, "status": "restarting"}, "Bot restarted successfully"
            )
        else:
...
def update_bot_v2(bot_id):
    ...
    try:
        ...
        # Валидация входных данных
        config_data = data.get("config", data) if "config" in data else data
        ...
        with cm.BOT_CONFIGS_LOCK:
            ...
            if "config" in data:
                bot_entry["config"].update(data["config"])
            else:
    ...
    try:
        with cm.BOT_CONFIGS_LOCK:
            ...
            if bot_entry.get("status") == "running":
                ...
                if not stop_success:
                    return api_response(
                        error=f"Failed to stop bot before deletion: {stop_message}", status_code=400
                    )
...
@app.route("/api/v2/telegram/validate-token", methods=["POST"])
@api_v2_auth_required
def validate_telegram_token_v2():
    ...
    try:
        ...
        return api_response(data=bot_data)
...
@app.route("/api/admin-bot", methods=["GET"])
@api_login_required
...
@app.route("/api/admin-bot", methods=["POST"])
@api_login_required
...
@app.route("/api/admin-bot", methods=["DELETE"])
@api_login_required
def stop_admin_bot():
