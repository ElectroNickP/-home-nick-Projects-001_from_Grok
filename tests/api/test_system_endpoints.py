...
import requests
import json
from tests.utils import test_logger, test_helper, assertion_helper, performance_monitor
...
class TestSystemEndpoints:
    ...
    def test_api_docs_endpoint(self, authenticated_session):
        ...
        assert "API Documentation" in content or "docs" in content.lower(), "Документация API не найдена"
    ...
    def test_endpoints_response_time(self, authenticated_session):
        ...
        for endpoint, max_time in endpoints:
            ...
            assert duration <= max_time, f"Endpoint {endpoint} превысил лимит времени: {duration}s > {max_time}s"
