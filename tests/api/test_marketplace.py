...
class TestMarketplaceAPI:
    ...
    def test_marketplace_bot_filtering(self, session_manager):
        ...
        for category in categories:
            ...
            for bot in data["data"]:
                assert bot.get("category") == category, f"Бот имеет неверную категорию: {bot.get('category')}"
    ...
    def test_marketplace_bot_search(self, session_manager):
        ...
        for bot in data["data"]:
            ...
            assert (search_lower in title_lower or 
                   search_lower in description_lower), f"Бот не содержит поисковый термин: {search_term}"
    ...
    def test_marketplace_bot_pagination(self, session_manager):
        ...
        test_logger.success(f"Пагинация работает: страница {pagination['page']}, всего страниц: {pagination['pages']}")
        ...
        for sort_field in sort_fields:
            ...
            if len(data["data"]) > 1:
                ...
                if values and all(isinstance(v, (int, float)) for v in values):
                    assert values == sorted(values), f"Значения не отсортированы по возрастанию: {values}"
    ...
    def test_marketplace_featured_bots(self, session_manager):
        ...
        for bot in data["data"]:
            assert bot.get("featured", False) == True, f"Бот не является избранным: {bot.get('title')}"
    ...
    def test_marketplace_api_performance(self, session_manager):
        ...
        for endpoint, description in endpoints:
            ...
            assert response.status_code == 200, f"{description} вернул статус {response.status_code}"
