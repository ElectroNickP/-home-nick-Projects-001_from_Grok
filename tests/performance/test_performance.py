...
class TestPerformance:
    ...
    def test_api_response_time(self, authenticated_session, performance_monitor):
        ...
        for endpoint in endpoints:
            ...
            assert performance_monitor.is_within_limit(1.0), f"Endpoint {endpoint} отвечает слишком медленно: {performance_monitor.get_duration():.2f}s"
    ...
    def test_concurrent_api_requests(self, authenticated_session, performance_monitor):
        ...
        for result in results:
            ...
            assert result['response_time'] < 2.0, f"Медленный ответ: {result['endpoint']} - {result['response_time']:.2f}s"
        
        # Вычисляем среднее время ответа
        avg_response_time = sum(r['response_time'] for r in results) / len(results)
        assert avg_response_time < 1.0, f"Среднее время ответа {avg_response_time:.2f}s превышает лимит 1.0s"
    ...
    def test_marketplace_performance(self, session_manager, performance_monitor):
        ...
        for endpoint in marketplace_endpoints:
            ...
            assert response.status_code == 200
            assert performance_monitor.is_within_limit(2.0), f"Маркетплейс {endpoint} отвечает слишком медленно: {performance_monitor.get_duration():.2f}s"
    
    def test_bot_operations_performance(self, authenticated_session, test_bot_data, performance_monitor):
        ...
        assert performance_monitor.is_within_limit(3.0), f"Создание бота слишком медленно: {performance_monitor.get_duration():.2f}s"
        ...
        assert performance_monitor.is_within_limit(1.0), f"Получение информации о боте слишком медленно: {performance_monitor.get_duration():.2f}s"
        ...
        assert performance_monitor.is_within_limit(2.0), f"Обновление бота слишком медленно: {performance_monitor.get_duration():.2f}s"
        ...
        assert performance_monitor.is_within_limit(2.0), f"Удаление бота слишком медленно: {performance_monitor.get_duration():.2f}s"
    ...
    def test_memory_usage(self, authenticated_session):
        ...
        assert memory_increase < 50, f"Подозрение на утечку памяти: увеличение на {memory_increase:.2f}MB"
    ...
    def test_scalability(self, authenticated_session):
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=load) as executor:
                ...
                for _ in range(load * 5):  # 5 запросов на поток
                    futures.append(executor.submit(
                        authenticated_session.get, "/api/v2/system/health"
                    ))
        ...
        for i, response_time in enumerate(response_times):
            assert response_time < 1.0, f"Производительность деградирует при нагрузке {load_levels[i]}: {response_time:.2f}s"
    ...
    def test_error_recovery_performance(self, authenticated_session):
        ...
        for endpoint in error_endpoints:
            ...
            assert end_time - start_time < 1.0, f"Обработка ошибки слишком медленная: {end_time - start_time:.2f}s"
            assert response.status_code in [404, 400, 500, 405], f"Неожиданный статус: {response.status_code}"
