...
class TestHelper:
    ...
    def validate_response(response: requests.Response, expected_status: int = 200):
        ...
        if response.status_code != expected_status:
            TestLogger.error(f"Неожиданный статус: {response.status_code}, ожидался: {expected_status}")
            return False
    ...
    @staticmethod
    def extract_bot_id_from_response(response: requests.Response) -> Optional[int]:
...
class AssertionHelper:
    ...
    def assert_bot_status(response: requests.Response, expected_status: str):
        ...
        try:
            ...
            assert actual_status == expected_status, f"Статус бота: {actual_status}, ожидался: {expected_status}"
        except json.JSONDecodeError:
            assert False, "Ответ не является валидным JSON"
    ...
    def assert_marketplace_bot(response: requests.Response, expected_enabled: bool = True):
        ...
        try:
            ...
            assert actual_enabled == expected_enabled, f"Маркетплейс: {actual_enabled}, ожидался: {expected_enabled}"
        except json.JSONDecodeError:
            assert False, "Ответ не является валидным JSON"
