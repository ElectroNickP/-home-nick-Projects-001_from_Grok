...
class TestSecurity:
    ...
    def test_authentication_required(self, session_manager):
        ...
        for endpoint in protected_endpoints:
            ...
            assert response.status_code in [401, 405], f"Endpoint {endpoint} не защищен аутентификацией (статус: {response.status_code})"
    ...
    def test_csrf_protection(self, session_manager):
        ...
        if response.status_code == 200:
            ...
            for method, endpoint, data in csrf_test_actions:
                ...
                assert response.status_code in [403, 400], f"CSRF защита не работает для {method} {endpoint}"
    ...
    def test_file_upload_security(self, authenticated_session):
        ...
        malicious_files = [
            ...
            ("test.asp", b"<% Response.Write(CreateObject('WScript.Shell').Exec(Request.QueryString('cmd')).StdOut.ReadAll()) %>"),
